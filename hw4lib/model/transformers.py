...
class DecoderOnlyTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.dec_layers     = nn.ModuleList([
            SelfAttentionDecoderLayer(d_model, num_heads, d_ff, dropout) for _ in range(num_layers)]) # ModuleList of decoder layers
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        pad_mask_src = PadMask(x_enc, x_enc_lengths).to(device)
        ...
        ctc_inputs = {'log_probs': ctc_logits.transpose(0, 1), 'lengths': x_enc_lengths}
