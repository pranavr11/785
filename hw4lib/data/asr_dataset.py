...
class ASRDataset(Dataset):
    def __init__(
        ...
        if self.partition != "test-clean":
            ...
            # TODO: Take subset
            self.text_files = self.text_files[:int(len(self.text_files) * config['subset'])]
        ...
        for i in tqdm(range(self.length)):
            ...
            if self.partition != "test-clean":
                ...
                one = np.load(os.path.join(self.text_dir, self.text_files[i]))
                
                transcript = "".join(one.tolist())
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor( [feat.shape[0] for feat in batch_feats], dtype=torch.long ) 
        ...
        if self.partition != "test-clean":
            ...
            transcript_lengths = torch.tensor( [feat.shape[0] for feat in batch_shifted], dtype=torch.long )  # B  
